...
# Importación para generar la factura en PDF
...
class RV(RecycleView):
    ...
    def eliminar_articulo(self):
        ...
        precio=0
        if indice>=0:
        	#self.layout_manager.deselect_node(self.layout_manager.selected_nodes[0])
    ...
    def actualizar_totales(self):
        sub_total = sum(item['precio_total'] for item in self.data)
        iva = sub_total * 0.14  # CAMBIO: Calcula el 14% del subtotal
        total = sub_total + iva  # CAMBIO: Suma subtotal y IVA
        
        if self.modificar_producto:
            self.modificar_producto(False, sub_total, iva, total) 
...
class PagarPopup(Popup):
    def __init__(self, total, pagado_callback, **kwargs):
        ...
        self.total = total  # Asegúrate de que total es un número flotante
        self.pagado = pagado_callback
        self.ids.total.text = "{:.2f}".format(self.total)  # Formatear como flotante
    ...
    def generar_factura_pdf(self, venta_id, productos):
        filename = f'factura_{venta_id}.pdf'
        c = canvas.Canvas(filename, pagesize=letter)
        c.drawString(100, 750, "Factura")
        c.drawString(100, 730, f"ID de Venta: {venta_id}")
        c.drawString(100, 710, f"Total: ${self.total}")

        y = 690
        for producto in productos:
            c.drawString(100, y, f"{producto['nombre']}: {producto['cantidad']} x ${producto['precio']} = ${producto['precio'] * producto['cantidad']}")
            y -= 20
    ...
    def confirmar_pago(self):
        ...
        venta_id = guardar_venta_en_bd()

        # Llama a la función para generar la factura en PDF
        self.generar_factura_pdf(venta_id, productos)

        # Llama al callback para confirmar el pago
        self.pagado()
...
class VentasWindow(BoxLayout):
    ...
    def eliminar_producto(self):
        ...
        self.ids.rvs.actualizar_totales() 

    def modificar_producto(self, cambio=True, sub_total=None, iva=None, total=None):
        if cambio:
            self.ids.rvs.modificar_articulo()
        else:
            self.ids.sub_total.text = f'$ {sub_total:.2f}'  # CAMBIO: Mostrar subtotal
            self.ids.iva.text = f'$ {iva:.2f}'  # CAMBIO: Mostrar IVA
            self.ids.total.text = f'$ {total:.2f}'
    ...
    def pagar(self):
        if self.ids.rvs.data:
            self.ids.rvs.actualizar_totales()  # CAMBIO: Asegúrate de actualizar los totales
            try:
                total = float(self.ids.total.text.replace('$', '').replace(',', '').strip())
            except ValueError:
                self.ids.notificacion_falla.text = 'Error al procesar el total'
                return
            popup = PagarPopup(total, self.pagado)
            popup.open()
        else:
    ...
    def pagado(self):
        ...
        venta_tuple = (self.ids.total.text, self.ahora, self.usuario['username'])
